...
function DiscountBotItem({
  ...
  const discountPrice =
    Number(price) + Number(parseInt(0.1 * price).toFixed(2)); // actual price to sold
  const purchaseReward = parseInt(0.02 * discountPrice).toFixed(2); //calculate reward point of item
  ...
  const handleClickOpenBalance = () => {
    if (discountPrice > balance) {
      ...
      dispatch(balanceCut(discountPrice));
      ...
      db.collection("notification").add({
        message: `Item purchased from website discount store. Balance (Rs) : ${discountPrice} /- has been deducted from your account.`,
      ...
      db.collection("transaction").add({
        ...
        message: "Item purchased from discount store. ( Balance )",
        signRP: true,
        costRP: Number(purchaseReward),
        signBalance: false,
        costBalance: Number(discountPrice),
        walletRP: Number(reward) + Number(purchaseReward),
        walletBalance: Number(balance) - Number(discountPrice),
      });
...
export default DiscountBotItem;
